...
describe('getTemplate', () => {
	...
	test('uses default template if not interactive and no template', async () => {
	...
	test('prompts if interactive and no template', async () => {
	...
	test('prompts if template is invalid', async () => {
		vi.mocked(prompts.select).mockResolvedValueOnce({
			variants: [{ name: 'v1', color: (s) => s }],
		});
