...
describe('getRunAppImmediately', () => {
	...
	test('should prompt to install harper if not installed and interactive', async () => {
		...
		expect(spawn.sync).toHaveBeenCalledWith('npm', ['install', '-g', 'harper'], expect.any(Object));
	...
	test('should continue if user chooses not to install harper', async () => {
		...
		expect(spawn.sync).not.toHaveBeenCalledWith('npm', ['install', '-g', 'harper'], expect.any(Object));
	});

	test('should use immediate if provided when not interactive', async () => {
		const result = await getRunAppImmediately(true, false, 'npm');
		expect(result).toEqual({ immediate: true, cancelled: false });
	});

	test('should use false if not interactive and no immediate', async () => {
	...
	test('should log error if harper installation fails', async () => {
		vi.mocked(spawn.sync)
			.mockReturnValueOnce({ status: 1 }) // Harper not installed
			.mockReturnValueOnce({ status: 1 }); // Install failed

		vi.mocked(prompts.confirm).mockResolvedValue(true);

		await getRunAppImmediately(undefined, true, 'npm');

		expect(prompts.log.error).toHaveBeenCalledWith('Failed to install Harper CLI.');
	});
});
