...
describe('parseArgv', () => {
	...
	test('detects version from positional arguments', () => {
		const result1 = parseArgv(['version']);
		expect(result1.version).toBe(true);
		expect(result1.targetDir).toBeUndefined();

		const result2 = parseArgv(['v']);
		expect(result2.version).toBe(true);
		expect(result2.targetDir).toBeUndefined();
	});
	...
	test('parses --no-interactive', () => {
		const result = parseArgv(['--no-interactive']);
		expect(result.interactive).toBe(false);
	});

	test('handles positional template without value', () => {
		const result = parseArgv(['template']);
