...
describe('getEnvVars', () => {
	...
	test('prompts if interactive and no argv', async () => {
		...
		expect(prompts.note).toHaveBeenCalledWith(expect.stringContaining('fabric.harper.fast'));
	...
	test('skips username prompt and note if provided via argv', async () => {
		vi.mocked(prompts.text).mockResolvedValue('prompttarget');
		vi.mocked(prompts.password).mockResolvedValue('promptpassword');

		await getEnvVars(true, 'vanilla', 'arguser');

		expect(prompts.note).not.toHaveBeenCalled();
		expect(prompts.text).toHaveBeenCalledTimes(1); // only target prompt should be called
	});
