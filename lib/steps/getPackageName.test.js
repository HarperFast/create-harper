...
describe('getPackageName', () => {
	...
	test('validate function returns error for invalid package name', async () => {
		vi.mocked(prompts.text).mockResolvedValue('valid-pkg');
		await getPackageName('My Project', true);
		const validate = vi.mocked(prompts.text).mock.calls[0][0].validate;
		expect(validate('valid-pkg')).toBeUndefined();
		expect(validate('Invalid Pkg')).toBe('Invalid package.json name');
