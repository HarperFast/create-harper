...
vi.mock('./steps/getProjectName.js', () => ({
	getProjectName: vi.fn().mockResolvedValue({ projectName: 'my-project', targetDir: 'my-dir', cancelled: false }),
}));
vi.mock('./steps/handleExistingDir.js', () => ({
	handleExistingDir: vi.fn().mockResolvedValue({ cancelled: false }),
}));
vi.mock('./steps/getPackageName.js', () => ({
	getPackageName: vi.fn().mockResolvedValue({ packageName: 'my-pkg', cancelled: false }),
}));
vi.mock('./steps/getTemplate.js', () => ({
	getTemplate: vi.fn().mockResolvedValue({ template: 'vanilla', cancelled: false }),
}));
vi.mock('./steps/getRunAppImmediately.js', () => ({
	getRunAppImmediately: vi.fn().mockResolvedValue({ immediate: true, cancelled: false }),
}));
vi.mock('./steps/getSkills.js', () => ({
	getSkills: vi.fn().mockResolvedValue({ selectedSkills: ['*'], selectedAgents: [], cancelled: false }),
}));
vi.mock('./steps/getEnvVars.js', () => ({
	getEnvVars: vi.fn().mockResolvedValue({ envVars: {}, cancelled: false }),
}));
...
describe('init.js', () => {
	beforeEach(async () => {
		...
		const { getSkills } = await import('./steps/getSkills.js');
		...
		vi.mocked(getSkills).mockResolvedValue({ selectedSkills: ['*'], selectedAgents: [], cancelled: false });
	...
	test('cancels if skills selection is cancelled', async () => {
		const { getProjectName } = await import('./steps/getProjectName.js');
		const { getTemplate } = await import('./steps/getTemplate.js');
		const { getEnvVars } = await import('./steps/getEnvVars.js');
		const { getRunAppImmediately } = await import('./steps/getRunAppImmediately.js');
		...
		vi.mocked(getEnvVars).mockResolvedValue({ envVars: {}, cancelled: false });
		vi.mocked(getRunAppImmediately).mockResolvedValue({ immediate: true, cancelled: false });
		vi.mocked(getSkills).mockResolvedValue({ cancelled: true });
		vi.mocked(parseArgv).mockReturnValue({});
		const prompts = await import('@clack/prompts');
