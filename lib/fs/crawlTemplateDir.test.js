...
vi.mock('node:fs');
vi.mock('./applyAndWriteTemplateFile.js');
...
describe(crawlTemplateDir, () => {
	...
	test('recursively crawls template directory and applies templates', () => {
		...
		expect(applyAndWriteTemplateFile).toHaveBeenCalledWith(
	...
	test('skips excluded files', () => {
		...
		crawlTemplateDir('target-dir', 'template-dir', { NAME: 'test' }, ['skip.txt']);
		...
		expect(applyAndWriteTemplateFile).not.toHaveBeenCalledWith(
			path.join('target-dir', 'skip.txt'),
			path.join('template-dir', 'skip.txt'),
			{ NAME: 'test' },
		);
	});
});
